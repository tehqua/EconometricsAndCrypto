    ...
    if crypto_prices:
        ...
        if selected_crypto:
            ...
            if crypto_history:
                ...
                # Chuyển đổi kiểu dữ liệu của cột 'close' sang float
                df['close'] = df['close'].astype(float)

                # Kiểm tra dữ liệu có đủ điều kiện để áp dụng econometrics không
                if len(df) >= 100:  # Điều kiện đủ điều kiện có thể điều chỉnh
...
# Tạo hàm main để xây dựng ứng dụng
def main():
